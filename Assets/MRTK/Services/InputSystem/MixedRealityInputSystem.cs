    ...
    public class MixedRealityInputSystem : BaseDataProviderAccessCoreSystem, IMixedRealityInputSystem, IMixedRealityCapabilityCheck
    {
        ...
        public void RaiseSourceDetected(IMixedRealityInputSource source, IMixedRealityController controller = null)
        {
            using (RaiseSourceDetectedPerfMarker.Auto())
            {
                ...
                if (controller != null)
                {
                    //NEW CODE TO """FIX""" PHANTOM HANDS
                    // This is not a good solution nor does it fix the original issue with the floating meshes but it means that the joint tracking works
                    foreach (IMixedRealityController c in DetectedControllers)
                        if (c.ControllerHandedness == controller.ControllerHandedness)
                            DetectedControllers.Remove(c);
